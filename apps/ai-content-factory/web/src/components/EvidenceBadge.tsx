 ...
 *
 * skill/工具返回的 EvidenceEnvelope 携带时效/置信度/来源元信息，
 * 本组件把它们渲染成紧凑徽章，让用户一眼判断证据可信度。
 *
 * 风格规范见 docs/24-conversational-streaming-style.md §3.4。
 * 从 apps/nexusops/web/src/components/EvidenceBadge.tsx 移植，类名改 .aicf-badge。
 */
...
export function EvidenceBadge({ data }: { data: EvidenceBadgeData }) {
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, alignItems: "center" }}>
      {data.freshness && (
        <span className="aicf-badge" style={freshnessStyle(data.freshness)}>
          {freshnessLabel(data.freshness)}
        </span>
      )}
      {data.confidence && (
        <span className="aicf-badge" style={confidenceStyle(data.confidence)}>
          {confidenceLabel(data.confidence)}
        </span>
      )}
      {data.source?.system && (
        <span className="aicf-badge" style={sourceStyle}>
          {data.source.system}
        </span>
      )}
      {data.caveat && (
        <span className="aicf-badge" style={caveatStyle} title={data.caveat}>
          ⚠ {data.caveat}
        </span>
      )}
    </div>
...
function confidenceLabel(c: Confidence): string {
  const icon = c === "measured" ? "✓" : c === "estimated" ? "≈" : "?";
