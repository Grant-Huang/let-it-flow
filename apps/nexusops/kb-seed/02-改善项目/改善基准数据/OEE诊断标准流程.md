...
| 最大损失项 | 阈值 | 首取证工具 | 典型根因方向 | 对策方向 |
|------------|------|------------|--------------|----------|
| 可用率 | <85% | `equipment.downtime` | 故障/换模/缺料 | 维护/SMED/补料 |
| 性能率 | <90% | `process.deviation` + 小停机 | 降速/小停机/参数漂移 | 调参/治小停机 |
| 质量率 | <95% | `quality.pareto` + `quality.cp_cpk` | 缺陷/返工/报废 | 调工艺/隔离/返工单 |
...
|--------|------|------|
| ≥0.85 | 健康 | 维持，常规保养 |
| 0.7-0.85 | 注意 | 监控，安排预防维护 |
| 0.6-0.7 | 预警 | 立即排查，安排纠正维护 |
| <0.6 | 危险 | 停机排查，故障风险高 |

> 健康分 < 0.6 时必须立即排查，不等保养周期（见主轴轴承异响案例的教训）。

## MTBF / MTTR 判定

| 指标 | 世界级 | 合格 | 预警 | 严重 |
|------|--------|------|------|------|
| MTBF (h) | ≥500 | 300-500 | 150-300 | <150 |
| MTTR (min) | ≤30 | 30-60 | 60-120 | >120 |

> MTBF 下降 > 30%（环比）即进入可靠性恶化期，应转向预测性维护。
...
|-----|------|------|------|
| ≥1.67 | 优秀 | 充足余量 | 维持 |
| 1.33-1.67 | 合格 | 满足规格 | 维持+监控 |
| 1.0-1.33 | 边际 | 能力勉强 | 改善系统 |
| <1.0 | 不足 | 严重不足 | 停机整改 / 100% 筛选 |
